#Name - Zubair Mohamed, Student ID - zm7162
...
class Molecule:
    ...
    def __init__(self, main_string):
        self.main_str = main_string

    def __str__(self):
        return self.main_str

    def __add__(self, second_str):
        
        total = str(self)+str(second_str)
        return Molecule(total)
    ...
    def _gettokens(self):
        ...
        return re.findall(r'[A-Z][a-z]*|\d+|\(|\)|\S', self.main_str)
    ...
    def _tokentree(self, target=None):
        ...
        if not target: target=self.main_str
    ...
    def check_symbols(self):
        ...
        for t in self._gettokens():
            if(t.isalpha() and (t not in _atomic_mass)):
                raise KeyError("There is no symbol representing" + t + "in the atomic table")
    ...
    def clean_copy(self):
        ...
        for token in tokens:
            if not correct(token):
                tokens.remove(token)
        return Molecule("".join(tokens))
    ...
    def atoms(self, symbol):
        ...
        """ 
        if symbol in _atomic_mass:
            num = 0
            temp = self._tokentree()
            while len(temp)!=0:
                token = temp.pop()
                if symbol == token[0]:
                    num += token[1]
                if type(token[0]) is list:
                    num_sym = 0
                    for i in token[0]:
                        if symbol == i[0]:
                            num_sym += i[1]
            ...
            raise KeyError("Symbol is not valid")
        return num

    #def replace(self, replMolecule):
    ...
    def __iter__(self):
        ...
        for i in self._gettokens():
            if i in _atomic_mass and i not in singles:
                tuples.append((i, self.atoms(i)))
                singles.append(i)
    ...
    def atomix(self):
        ...
        temp = []
        iterator = []
        before = list(counter for counter in self)
        while counter!= len(before):
            temp.append((before[counter][0],_atomic_mass[before[counter][0]]*before[counter][1]))
            counter += 1
        temp.sort(key = lambda x: float(x[1]), reverse = True)
        counter = 0
        while counter!= len(temp):
            iterator.append((temp[counter][0],temp[counter][1]/_atomic_mass[temp[counter][0]]))
            counter += 1
        counter = 0
        while counter != len(iterator):
            yield(Molecule(iterator[counter][0]+str(int(iterator[counter][1]))))
            counter += 1


    def replace(self, a, b, num = None):
        if num == None:
            return Molecule(self.main_str.replace(a, b))
        return Molecule(self.main_str.replace(a, b, num))
